...
Screenstory.prototype.loadWebdrivercss = function loadWebdrivercss(image) {
    ...
    image.fileNew = this.generatePath(image.capabilities, image.storyId, image.id + '.new.png');
    image.fileOriginal = this.generatePath(image.capabilities, image.storyId, image.id + '.current.png');
    image.fileDiff = this.generatePath(image.capabilities, image.storyId, path.join('diff', image.id) + '.diff.png');
    image.fileDocument = this.generatePath(image.capabilities, image.storyId, image.id + '.png');
...
Screenstory.prototype.save = function save(image, options, cb) {
    ...
    if (!options.capabilities) {
        return cb(new VError('Option "capabilities" is required. Just provide '));
    }
...
function prepareData(destFile, data) {
    ...
    data = _.map(data, function prepareDataItem(story) {
        // Replace base64 encoded url with file url
        if (story.fullpath) {
            story.url = path.relative(destPath, story.fullpath);
        }
        if (story.fileNew) {
            story.urlNew = path.relative(destPath, story.fileNew);
        }
        if (story.fileOriginal) {
            story.urlOriginal = path.relative(destPath, story.fileOriginal);
        }
        if (story.fileDiff) {
            story.urlDiff = path.relative(destPath, story.fileDiff);
        }
        if (story.fileDocument) {
            story.urlDocument = path.relative(destPath, story.fileDocument);
        }
        return story;
    });
